...
mutual
  ...
  lcheck rig erase env (Ref fc nt fn)
      = do logC "quantity" 15 $ do pure "lcheck Ref \{show (nt)} \{show !(toFullNames fn)}"
           ...
           pure (Ref fc nt fn, gnf env (embed ty), Usage.empty)
  ...
  discharge : {vars : _} ->
              ...
              Term (Scope.bind vars nm) -> Glued (Scope.bind vars nm) -> Usage vars ->
              Core (Term vars, Glued vars, Usage vars)
  ...
  expandMeta rig erase env n idx (PMDef _ [<] (STerm _ fn) _ _) args
    ...
    where
      substMeta : {drop, vs : _} ->
                  Term (Scope.addInner vs drop) -> List (Term vs) ->
  ...
  lcheckMeta rig erase env fc n idx [] chk nty
      = do defs <- get Ctxt
           pure (Meta fc n idx (reverse chk), glueBack defs env nty, Usage.empty)
